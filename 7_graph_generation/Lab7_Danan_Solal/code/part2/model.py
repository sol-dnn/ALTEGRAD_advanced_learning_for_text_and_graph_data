...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.fc:
            x = layer(x)
        x = self.fc_proj(x)
        T2 = torch.reshape(x, (-1, self.n_nodes, self.n_nodes))
        T2_symmetric = (T2 + T2.transpose(-1, -2)) / 2
        adj = torch.sigmoid(T2_symmetric)
        ##################
...
class Encoder(nn.Module):
    ...
    def forward(self, adj, x, idx):
        ############## Task 8
        ## be carfull with the dhancy matrix
    
        ##################
        # First message passing layer
        x = torch.mm(adj, x)  
        x = self.mlps[0](x)   #  first MLP

        # Second message passing layer
        x = torch.mm(adj, x)
        x = self.mlps[1](x)   # second MLP
        ##################
...
class VariationalAutoEncoder(nn.Module):
    ...
    def loss_function(self, adj, x, idx, y, beta=0.05):
        ...
        ############## Task 9
        mu = self.fc_mu(x_g)  # Mean vector
        logvar = self.fc_logvar(x_g)  # Log variance vector
